...
>
> 研究 runner 目前收集快照而非生成论点；平台仲裁/质量评分是启发式实现。Workflow 的审批恢复、发布节点、并发限制与失败终态仍需完善。本仓库现有 loopback-only fixture 身份和本地 D1 开发路径，但没有可直接替换为普通公网 Node 服务的独立生产认证方案。A3.2 已交付请求级 Workspace RBAC（`requireWorkspaceAccess` / `requirePortfolioAccess` / `listAccessibleWorkspaces` 统一 401/403/404，跨租户资源 ID 返回与缺失相同的 404，成员管理 API 写脱敏审计）、原子化控制性 Owner 转移（目标须为已有成员），以及双租户 HTTP 集成测试。A3.3 已交付可恢复研究任务状态机（每次领取写入唯一 lease token，所有终结/重试/取消状态转换在同一个 D1 原子 batch 中完成：状态变更、`event_seq` 递增与对应 `research_job_events` 写入不分离，compare-and-set 校验 token、预期状态、未过期 lease 与 `cancel_requested_at`；取消确定性地进入 `cancelled` 终态而非伪 succeeded；lease 过期恢复、attempts 耗尽、timeout 与 cancelled recovery 均在同一原子边界写对应 `lease_expired_recovered`/`failed`/`cancelled` event，timeline 中不留无记录的状态变化）、at-least-once 的 Webhook Outbox（成功状态与 webhook delivery fan-out 在同一事务提交，delivery 以条件化 SQL 依赖同批次的成功 CAS，原始响应体不落库，owner-only dead-letter 重投），以及安全的账户删除后台工作流（每个副作用 SQL 的 `WHERE` 直接携带 fence 条件——request 处于 processing、lease token 匹配且未过期、属于目标用户、且用户无共享 controlling ownership——不存在先读后写的 TOCTOU 窗口；仅 `requested` 可取消，个人/共享 Workspace 区分）。通知 Outbox 目前只有入队与投递逻辑、尚未达到 Webhook 同级的 lease/CAS 保护，因此不宣称「可靠投递通知」。仍未完成：托管队列、生产 OAuth/SSO、监控告警、Webhook 崩溃恢复、通知的 Webhook 级可靠投递与合规审批的数据保留策略。
