...
- 首页研究台、论点卡包含示例数据；研究任务完成后，尚未把真实结果完整接入这些卡片。
...
| 研究任务 | D1 队列、幂等、事件、证据版本和研究快照 | 真实结果展示、论点生成、恢复与租户端到端验证 |
...
1. 研究结果展示实际快照、来源、时间和缺失信息，并明确区分示例与真实数据。
2. 提供可重复的本地数据库初始化、认证配置与完整用户旅程。
3. 增加租户隔离/任务恢复端到端测试，加固出站请求、删除流程及 Workflow 生命周期。
