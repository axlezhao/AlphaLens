...
1. Harden workflow recovery, outbound requests and deletion. A3.2 delivered request-level RBAC (`requireWorkspaceAccess` / `requirePortfolioAccess` / `listAccessibleWorkspaces`), dual-tenant HTTP tests, member management APIs, and atomic controlling-owner transfer. A3.3 delivered the recoverable research-job state machine (atomic lease claim, bounded retry, lease recovery), the at-least-once webhook/notification outbox (with SSRF-safe URL validation), and a safe background account-deletion workflow. Still open: a managed queue host, production OAuth/SSO, monitoring/alerting and compliance-approved data-retention review.
