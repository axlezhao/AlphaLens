...
- **A3.3: research state, events and outbox are one atomic transaction** — every critical state transition (`queued`, `running`, `retrying`, `succeeded`, `failed`, `cancelled`) now writes the state change, the `event_seq` increment and the matching `research_job_events` row inside a single `db.batch`. The `succeeded` transition additionally writes its webhook-delivery fan-out in the same batch, with each delivery `INSERT` conditionally gated on the job actually reaching `succeeded` (`WHERE EXISTS (... status='succeeded')`), so a crash between the state update and the outbox can no longer leave a succeeded job without a recoverable business event. Event and delivery inserts are `INSERT OR IGNORE` (deterministic ids / unique sequence) so retries never duplicate terminal events.
- **A3.3: deterministic cancellation** — when the guarded `succeeded` CAS fails, the worker now distinguishes cancelled from stale: if a cancel flag is present and this worker still holds an unexpired lease, it atomically finalises the job `cancelled` (with a single `cancelled` event and no `research.completed` delivery); otherwise it returns `stale`/`lease_lost` without writing any state or event. `recoverExpiredJobs` now respects `cancel_requested_at`, moving an expired cancelled running job to `cancelled` (with event) instead of `retrying`/`failed`.
- **A3.3: account deletion side effects fenced atomically** — `executeDeletion` no longer performs a select-then-write fence. Each side effect (user tombstone, membership removal, personal-workspace tombstone) carries the same fence condition directly in its `WHERE` clause (`EXISTS (deletion request is processing, holds this worker's unexpired lease token, belongs to the target user, and the user has no shared controlling ownership)`). If any condition fails, every statement affects zero rows and nothing is deleted. Owner membership is never deleted directly (a DB trigger forbids it); the personal workspace is tombstoned instead.
